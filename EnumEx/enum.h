...
namespace compile_time
{
    ...
    template<auto u>
    using ctint = typename std::integral_constant<decltype(u), u>;

    template<auto u>
    constexpr ctint<u> ctintvar{}; // cintvar is a templatized variable.
...
struct EnumType : public ENUMSTRUCT
{
    ...
    static constexpr size_t indexOf()
    {
        ...
        if constexpr(std::is_same_v<ctint<ENUMVAL>, std::tuple_element_t<i, TUPLE>>)
            return i;
        else
...
namespace impl
{
    struct example{ enum Type { Red, Green, Blue, Orange, DEFAULT = Blue}; };
    struct ColorTypes { enum Type { Red, Green, Blue, Orange, DEFAULT = Blue }; };
} // namespace impl

#define V(x) ctintvar<T::x>


#define T impl::ColorTypes
...
#undef T
using ColorTypes = decltype(color_type);
using example = decltype(color_type);
// ---[ Transformation
namespace impl
{
